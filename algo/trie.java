class trie
{
	class node
	{
		...
		node next,prev;
		list children;
		void recurse(String pattern,int index,int nodeindex,int counter,boolean state,int length)
		{
			if(state)
			{
				if(children==null)
					addsolution(start-counter,start-counter+length-1);
				else	
				{
					node x = children.header.next;
					while(x!=children.trailer)
					{
						x.recurse(pattern,index,x.start,counter+(end-start+1),true,length);
						x=x.next;
					}
				}
			}
				...
				if(pattern.charAt(index)=='*')
				{
					if(nodeindex>end)
					{
						if(children!=null)
						{
							...
							while(x!=children.trailer)
							{
								x.recurse(pattern,index,x.start,counter+(end-start+1),false,length);
								x=x.next;
							}
						}
					}
					else
						recurse(pattern,index,nodeindex+1,counter,false,length+1);
				}
				if(nodeindex<=end && nodeindex<len)
				{
					if((pattern.charAt(index)=='*' || pattern.charAt(index)=='?') || pattern.charAt(index)==str.charAt(nodeindex))
					{
						...
						if(index==pattern.length())
							this.recurse(pattern,index,start,counter,true,length+1);
						else if(nodeindex>end)
						{
							if(children!=null)
							{
								...
								while(x!=children.trailer)
								{
									x.recurse(pattern,index,x.start,counter+(end-start+1),false,length+1);
									x=x.next;
								}		
							}			
						}
						else
							recurse(pattern,index,nodeindex,counter,false,length+1);
					}
				}
			}
		}
			...
			if(t!=ref.trailer)
			{
				if(str.charAt(beg)==str.charAt(t.start))
				{
					...
					if(a==fin+1)
					{
						...
						temp1.prev=t.children.trailer.prev;
	...
	void lookup(String pattern)
	{
		...
		{
			x.recurse(pattern,0,x.start,0,false,0);
			x=x.next;
		}
